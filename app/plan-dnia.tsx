...
const DEFAULT_PLAN_PLACEHOLDERS = ['Dla siebie', 'Obowiązki', 'Dla relacji'];
...
function emptyPlan(dateKey: DateKey): DailyPlan {
  return {
    dateKey,
    planText: '',
    items: DEFAULT_PLAN_PLACEHOLDERS.map((_, index) => ({ id: `${dateKey}_${index + 1}`, text: '', done: false })),
    self: '',
    duties: '',
    relations: '',
    challenge: '',
...
function sanitizePlan(value: Partial<DailyPlan> | undefined, dateKey: DateKey): DailyPlan {
  ...
  if (items.length === 0) {
    ...
    const hasAnyText = items.some((item) => item.text.trim().length > 0);
    if (!hasAnyText && items.length < DEFAULT_PLAN_PLACEHOLDERS.length) {
      const next = [...items];
      for (let i = items.length; i < DEFAULT_PLAN_PLACEHOLDERS.length; i += 1) {
        next.push({ id: `${dateKey}_${i + 1}`, text: '', done: false });
      }
      items = next;
    }
  }
    ...
    <BackgroundWrapper>
      ...
      >
        ...
        <View style={styles.instructionsWrap}>
          <Image source={Watermark} resizeMode="contain" style={styles.instructionsWatermark} />
          <View style={styles.instructionsHeader}>
            ...
            <Pressable style={styles.instructionsToggleBtn} onPress={toggleInstruction}>
              <Text numberOfLines={1} ellipsizeMode="tail" style={styles.instructionsToggleText}>
                {instructionOpen ? 'Ukryj instrukcję' : 'Pokaż instrukcję'}
              </Text>
            </Pressable>
          </View>
          <Text
            style={[styles.instructionsLead, compact && !instructionOpen && styles.instructionsLeadCompact]}
            numberOfLines={compact && !instructionOpen ? 1 : undefined}
            ellipsizeMode="tail"
          >
            {compact && !instructionOpen ? 'Zaplanuj dzień i wieczorem go podsumuj.' : PLAN_INSTRUCTION_SHORT}
          </Text>
          {instructionOpen && (
...
const styles = StyleSheet.create({
  ...
  content: { padding: 18, paddingTop: 56, paddingBottom: 40, position: 'relative' },
  bgOrbA: {
    position: 'absolute',
    width: 270,
    height: 270,
  ...
  instructionsHeaderTitle: { color: 'white', fontSize: 17, fontWeight: '800', flex: 1, minWidth: 0 },
  ...
  instructionsToggleText: { color: '#AEE1FF', fontSize: 12, fontWeight: '700' },
  instructionsLead: { color: SUB, fontSize: 15, lineHeight: 22, marginTop: 10 },
  instructionsLeadCompact: { marginTop: 8, fontSize: 14, lineHeight: 18 },
