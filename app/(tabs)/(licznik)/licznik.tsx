...
export default function LicznikScreen() {
  ...
  const calculateStats = (startDate: Date) => {
    ...
    setStats({ years, months, days, totalDays: totalDays > 0 ? totalDays : 0 });
  };
  ...
  const handleReset = () => {
    Alert.alert(
      'Zresetować licznik?',
      'To wyzeruje licznik i rocznice. Będzie można ustawić nową datę.',
      [
        { text: 'Anuluj', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            await AsyncStorage.removeItem('unlockedAngels');
            setDate(now);
            calculateStats(now);
          },
        },
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.captureArea}>
          ...
          <Text style={styles.headerSubtitle}>
            Każdy z nas lubi wiedzieć jak długo zdrowieje. 
            Mimo tego, co mówię, że: "To nie ważne", to jednak dobrze znam datę rozpoczęcia tej drogi. A Twoja trwa już:
          </Text>
        ...
        <Pressable style={styles.button} onPress={() => setShow(!show)}>
          ...
          <Text style={styles.buttonText}>
            {show ? "Zwiń ustawienia" : "Ustaw datę początkową"}
          </Text>
        </Pressable>
        ...
        <Pressable onPress={() => router.push('/(licznik)/licznik-nagrody')} style={styles.rewardsTile}>
          <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  statBox: { flex: 1, backgroundColor: 'rgba(255,255,255,0.05)', margin: 5, padding: 18, borderRadius: 15, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(120, 200, 255, 0.1)' },
  statValue: { ...TYPE.h2, color: 'white' },
  statLabel: { ...TYPE.bodySmall, color: 'rgba(255,255,255,0.6)', marginTop: 6 },
  button: { flexDirection: 'row', backgroundColor: 'rgba(120, 200, 255, 0.2)', paddingVertical: 15, borderRadius: 12, alignItems: 'center', borderWidth: 1, borderColor: '#78C8FF', width: '100%', justifyContent: 'center' },
  ...
  rewardsTile: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.05)', width: '100%', padding: 20, borderRadius: 15, borderWidth: 1, borderColor: 'rgba(120, 200, 255, 0.22)', marginBottom: 24 },
  rewardsTitle: { ...TYPE.h3, color: 'white' },
  rewardsSubtitle: { ...TYPE.bodySmall, color: 'rgba(255,255,255,0.7)', marginTop: 4 },
  pickerContainer: { backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 20, marginTop: 10, padding: 10, width: '100%', borderWidth: 1, borderColor: 'rgba(120, 200, 255, 0.1)' },
